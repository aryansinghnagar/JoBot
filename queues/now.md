...
- [x] **WS6 (gate G6) — DONE 2026-08-16 (PASS_LOCAL):** GUI Human-in-the-loop Approval Inbox (`Approvals.jsx`), live ATS site health cockpit (`Health.jsx`), extended stdio sidecar RPC methods (`approvals_list`, `approvals_decide`, `site_health`, `evidence_manifest`, `candidate_facts`, `import_resume`), 20 vitest GUI tests green.
- [x] **WS7 (gate G7) — DONE 2026-08-16 (PASS_LOCAL):** Atomic SQLite hot backup and disaster recovery harness (`jobot db backup`, `jobot db restore`), chaos and crash-after-send failure injection suite (UC-40), soak and continuous high-throughput simulation suite (UC-41); 667 pytest tests green.
