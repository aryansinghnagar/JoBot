...
- **Data & Storage**: Local filesystem-first state (Markdown momentum queues) + SQLite WAL database (`~/.jobaut/jobaut.db`, mode 0600) + `age` encrypted profile & credentials (`profile.yaml`) integrated with OS Keyring.
...
- **Phases**:
  ...
  2. **Phase 0.1.1**: Scaffolding (`pyproject.toml`, `pytest`, `ruff`, `mypy`, `.gitignore`, `AGENTS.md`, `README.md`, `~/.jobaut/` structure)
