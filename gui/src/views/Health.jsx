...
export function Health({ rpc }) {
  ...
  return (
    <section className="view">
      ...
      <p className="muted">
        Real-time availability, success rates, latency, and circuit breaker status across all supported job sites.
      </p>
            ...
            <tbody>
              {sites.map((s) => (
                <tr key={s.site} style={{ borderBottom: "1px solid #222" }}>
                  <td style={{ padding: "0.5rem", fontWeight: "bold" }}>{s.site}</td>
                  ...
                  <td style={{ padding: "0.5rem" }}>{s.success_count + s.failure_count}</td>
                  <td style={{ padding: "0.5rem" }}>{(s.success_rate * 100).toFixed(1)}%</td>
                  <td style={{ padding: "0.5rem" }}>{s.consecutive_failures}</td>
                  <td style={{ padding: "0.5rem", color: "#888", fontSize: "0.85rem" }}>
                    {s.last_error || "None"}
                  </td>
                </tr>
              ))}
