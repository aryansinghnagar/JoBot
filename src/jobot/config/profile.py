...
def profile_config_path(name: str | None = None, profile_dir: Path | None = None) -> Path:
    return (profile_dir or DEFAULT_PROFILE_DIR) / f"{_resolve_name(name)}.yaml"


def load_profile_config(name: str | None = None, profile_dir: Path | None = None) -> ProfileConfig:
...
def load_llm_settings(name: str | None = None) -> LLMConfig:
...
def task_override_map(name: str | None = None) -> dict[str, dict[str, Any]]:
