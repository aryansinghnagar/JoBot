...
class NaukriSubmitter:
    ...
    async def submit(self, application: Application, page: Optional[Any] = None) -> bool:
        ...
        logger.info(
            f"[NAUKRI SUBMIT] Submitting application {application.application_id[:8]} for job {application.job_id}"
        )
        ...
        if page is not None and hasattr(page, "click"):
            try:
                ...
                logger.info(
                    f"[NAUKRI STEALTH SUBMIT] Simulating mouse trajectory over {len(curve)} Bezier curve points to apply button"
                )
