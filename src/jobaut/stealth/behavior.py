...
class BehavioralMimicry:
    ...
    def generate_bezier_curve(
        ...
        # Generate control points offset from straight line
        mid_x = (x1 + x2) / 2
        mid_y = (y1 + y2) / 2
        ctrl1_x = x1 + (mid_x - x1) * random.uniform(0.2, 0.8) + random.randint(-50, 50)
        ctrl1_y = y1 + (mid_y - y1) * random.uniform(0.2, 0.8) + random.randint(-50, 50)
        ...
        for i in range(steps + 1):
            ...
            x = (1 - t) ** 3 * x1 + 3 * (1 - t) ** 2 * t * ctrl1_x + t**3 * x2
            y = (1 - t) ** 3 * y1 + 3 * (1 - t) ** 2 * t * ctrl1_y + t**3 * y2
