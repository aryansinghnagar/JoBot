...
from jobaut.models.domain import ApplicationStatus, CompensationDetails, PersonalInfo, UserProfile
...
app = typer.Typer(name="jobaut", help="Autonomous Job Application Operating System CLI")
console = Console()
...
def setup() -> None:
    ...
    console.print("[bold green][OK] Running jobaut Setup Wizard...[/bold green]")
    db = DatabaseManager()
    ...
    console.print("[bold blue][OK] Setup complete! Add your profile with 'jobaut profile init'[/bold blue]")
...
def profile_cmd(
    ...
    vault = CredentialVault()
    profile_dir = Path.home() / ".jobaut" / "profiles"
    ...
    if action == "init":
        ...
        if not profile_path.exists():
            console.print("[yellow]No profile found. Run 'jobaut profile init' first.[/yellow]")
            return
...
def auto_apply_cmd(
    ...
    console.print(f"[bold cyan]jobaut Discovery Engine: Searching for '{target_title}' across portals [{portals}]...[/bold cyan]")
    vault = CredentialVault()
    db = DatabaseManager()
    profile_path = Path.home() / ".jobaut" / "profiles" / "default.enc"
...
def run_cmd(
    ...
    console.print(f"[bold cyan]jobaut: Applying to job posting at {job_url} on site '{site}'[/bold cyan]")
    vault = CredentialVault()
    db = DatabaseManager()
    profile_path = Path.home() / ".jobaut" / "profiles" / "default.enc"
...
def export_cmd() -> None:
    ...
    console.print("[bold green][OK] Diagnostic package exported to ~/.jobaut/backups/[/bold green]")
